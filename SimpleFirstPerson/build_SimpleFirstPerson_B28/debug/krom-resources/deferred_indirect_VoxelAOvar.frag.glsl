...
float traceAO(vec3 origin, vec3 normal, sampler3D voxels_1)
{
    ...
    vec3 c2 = (o1 - o2) * 0.5;
    vec3 param = normal;
    float _420 = traceConeAO(voxels_1, origin, param, 0.557851731777191162109375, 3.4641015529632568359375);
    float col = _420;
    vec3 param_1 = mix(normal, o1, vec3(0.5));
    float _425 = traceConeAO(voxels_1, origin, param_1, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _425;
    vec3 param_2 = mix(normal, o2, vec3(0.5));
    float _432 = traceConeAO(voxels_1, origin, param_2, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _432;
    vec3 param_3 = mix(normal, -c1, vec3(0.5));
    float _440 = traceConeAO(voxels_1, origin, param_3, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _440;
    vec3 param_4 = mix(normal, -c2, vec3(0.5));
    float _448 = traceConeAO(voxels_1, origin, param_4, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _448;
    return (col / 5.0) * 1.0;
}
...
void main()
{
    ...
    {
        _483 = octahedronWrap(g0.xy);
    }
    n = vec3(_483.x, _483.y, n.z);
    n = normalize(n);
    ...
    vec3 _640 = fragColor.xyz * texture(ssaotex, texCoord).xyz;
    fragColor = vec4(_640.x, _640.y, _640.z, fragColor.w);
}
