...
float traceAO(vec3 origin, vec3 normal, sampler3D voxels_1)
{
    ...
    vec3 c2 = (o1 - o2) * 0.5;
    vec3 param = normal;
    float _390 = traceConeAO(voxels_1, origin, param, 0.557851731777191162109375, 3.4641015529632568359375);
    float col = _390;
    vec3 param_1 = mix(normal, o1, vec3(0.5));
    float _395 = traceConeAO(voxels_1, origin, param_1, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _395;
    vec3 param_2 = mix(normal, o2, vec3(0.5));
    float _402 = traceConeAO(voxels_1, origin, param_2, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _402;
    vec3 param_3 = mix(normal, -c1, vec3(0.5));
    float _410 = traceConeAO(voxels_1, origin, param_3, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _410;
    vec3 param_4 = mix(normal, -c2, vec3(0.5));
    float _418 = traceConeAO(voxels_1, origin, param_4, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _418;
    return (col / 5.0) * 1.0;
}
...
void main()
{
    ...
    {
        _453 = octahedronWrap(g0.xy);
    }
    n = vec3(_453.x, _453.y, n.z);
    n = normalize(n);
    ...
    vec3 voxpos = p / vec3(8.0);
    envl *= (1.0 - traceAO(voxpos, n, voxels));
    fragColor = vec4(envl.x, envl.y, envl.z, fragColor.w);
    vec3 _553 = fragColor.xyz * texture(ssaotex, texCoord).xyz;
    fragColor = vec4(_553.x, _553.y, _553.z, fragColor.w);
}
